...
def get_args():
    ...
    parser.add_argument("--envs_folder", required=True)
    ...
    parser.add_argument("--using_obstacle_avoidance", required=True, type=bool)
...
class DirToAxisTestingNode:
    ...
    def __init__(
        ...
        max_vel,
        max_ang_vel,
        using_obstacle_avoidance,
    ):
        self.envs_folder = envs_folder
        self.n_trials_per_env = n_trials_per_env
        self.max_vel = max_vel
        self.max_ang_vel = max_ang_vel
        self.using_obstacle_avoidance = using_obstacle_avoidance
        self.env_names = os.listdir(self.envs_folder)
        self.env_names.sort()
        self.init_ros_node()
    ...
    def run(self):
        ...
        for env_name in self.env_names:
            ...
            path_to_env_folder = os.path.join(self.envs_folder, env_name)
            folder_handler = EnvFolderHandler(path_to_env_folder)
            ...
            info = {
                ...
                "using_obstacle_avoidance": self.using_obstacle_avoidance,
            }
            for n_test in range(self.n_trials_per_env):
                ...
                while True:
                    ...
                    if current_pose[3] > np.deg2rad(40) or current_pose[4] > np.deg2rad(
                        40
                    ):
